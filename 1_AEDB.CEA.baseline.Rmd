...
AEDB.tableOne = print(CreateTableOne(vars = basetable_vars, 
                          ...
                          quote = FALSE, noSpaces = TRUE, showAllLevels = TRUE, explain = TRUE, 
                          format = "pf", 
                          contDigits = 3)
```
...
AEDB.full.tbl <- AEDB.full %>%
  select(all_of(basetable_vars), Gender) %>%
  tbl_summary(
    by = Gender,
    missing = "ifany",
...
AEDB.full.tbl_df <- AEDB.full.tbl %>% as_tibble()

# Save to Excel
write.xlsx(AEDB.full.tbl_df,
...
# Convert to a clean data frame
AEDB.CEA.tbl_df <- AEDB.CEA.tbl %>% as_tibble()

# Save to Excel
write.xlsx(AEDB.CEA.tbl_df,
           file = paste0(BASELINE_loc, "/",Today,".",PROJECTNAME,".AE.CEA.BaselineTable.xlsx"), 
           rowNames = FALSE, 
           colNames = TRUE, 
           sheetName = "AE_CEA", overwrite = TRUE)
```
...
ft <- gtsummary::as_flex_table(AEDB.full.tbl) %>%
  theme_vanilla() %>%
  autofit() %>%
...
# Save to Word
doc <- read_docx() %>%
  body_add_par("Table 1. Baseline characteristics full Athero-Express Biobank Study") %>%
  body_add_flextable(ft)
print(doc, target = paste0(BASELINE_loc, "/",Today,".",PROJECTNAME,".AE.BaselineTable.docx"))

# Print message
message("Table exported successfully to *.AE.BaselineTable.xlsx and *.AE.BaselineTable.docx with Calibri font and size 9.")
rm(ft, doc)


# Format for Word output with Calibri font and size 9
ft <- gtsummary::as_flex_table(AEDB.CEA.tbl) %>%
  theme_vanilla() %>%
  autofit() %>%
  set_table_properties(layout = "autofit") %>%
  flextable::font(fontname = "Calibri", part = "all") %>%
...
# Save to Word
doc <- read_docx() %>%
  body_add_par("Table 1. Baseline characteristics Athero-Express Biobank Study -- CEA only") %>%
  body_add_flextable(ft)
print(doc, target = paste0(BASELINE_loc, "/",Today,".",PROJECTNAME,".AE.CEA.BaselineTable.docx"))

# Print message
message("Table exported successfully to *.AE.CEA.BaselineTable.xlsx and *.AE.CEA.BaselineTable.docx with Calibri font and size 9.")
rm(ft, doc)
```
...
------------------------------------------------------------------------------------------------------------------------
    ...
    **Changes log**
    * v1.3.3 Fixed an issue where the Word-document output was not working properly.
