...
In this notebook we create a baseline table of the whole cohort, of the CEA-cohort, and the subset used for the b37 eQTL analyses. 
...
Also writing it to a Word-document.
...
# Create Table 1 and store as a data frame
AEDB.tableOne = print(CreateTableOne(vars = c(basetable_vars),
                                     factorVars = basetable_bin,
                                     # strata = "StudyName",
                                     data = AEDB, 
                                     includeNA = TRUE), 
                      ...
                      contDigits = 3) %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "Variable")  # Convert row names to column

# Extract clean variable names from CreateTableOne metadata
var_names <- names(CreateTableOne(
                  vars = c(basetable_vars),
                  factorVars = basetable_bin,
                  # strata = "StudyName",
                  data = AEDB, 
                  includeNA = TRUE)$MetaData$varNames)

# Replace the "Variable" column with actual variable names
AEDB.tableOne$Variable <- var_names

# View output
head(AEDB.tableOne)

# Convert to a properly formatted data frame
table_one_df <- print(AEDB.tableOne, 
                      quote = FALSE, showAllLevels = TRUE,
                      format = "p", contDigits = 3) %>%
                as.data.frame()

# Clean up variable names
table_one_df <- rownames_to_column(table_one_df, var = "Variable")  # Convert row names to column
table_one_df$Variable <- var_names  # Replace technical row names with actual variable names

# Save to Excel
write_xlsx(table_one_df, path = "TableOne.xlsx")

# Format for Word output with Calibri font and size 9
ft <- flextable(table_one_df) %>%
  theme_vanilla() %>%
  autofit() %>%
...
print(doc, target = paste0(BASELINE_loc, "/",Today,".",PROJECTNAME,".AE.BaselineTable.docx"))

# Print message
message("Table exported successfully to TableOne.xlsx and TableOne.docx with Calibri font and size 9.")
rm(table_one_df, ft, doc)
```

```{r}
# Create Table 1 and store as a data frame
AEDB.CEA.tableOne = print(CreateTableOne(vars = c(basetable_vars),
                                     factorVars = basetable_bin,
                                     # strata = "StudyName",
                                     data = AEDB.CEA, 
                                     includeNA = TRUE), 
...
# Replace the "Variable" column with actual variable names
AEDB.CEA.tableOne$Variable <- var_names

# View output
head(AEDB.CEA.tableOne)

# Convert to a properly formatted data frame
table_one_df <- print(AEDB.CEA.tableOne, 
                      quote = FALSE, showAllLevels = TRUE,
                      format = "p", contDigits = 3) %>%
                as.data.frame()
...
doc <- read_docx()
doc <- body_add_flextable(doc, ft)
print(doc, target = paste0(BASELINE_loc, "/",Today,".",PROJECTNAME,".AE.CEA.BaselineTable.docx"))
...
------------------------------------------------------------------------------------------------------------------------
    ...
    **Changes log**
    * v1.3.2 Add Word-document output. 
