...
Let's prepare the `tableOne` object and output it to Excel.
...
AEGS_eQTL.tableOne = print(CreateTableOne(vars = c(basetable_vars),
                                     factorVars = basetable_bin,
                                     # strata = "StudyName",
                                     data = AEGS_eQTL, 
                                     includeNA = TRUE), 
...
AEGS_eQTL.tableOne$Variable <- var_names

# View output
# head(AEGS_eQTL.tableOne)
```
...
write.xlsx(file = paste0(BASELINE_loc, "/",Today,".",PROJECTNAME,".AEGS_eQTL.BaselineTable.xlsx"), 
           format(as.data.frame(AEGS_eQTL.tableOne), digits = 5, scientific = FALSE),
           rowNames = TRUE, 
           colNames = TRUE, 
...
table_one_df <- print(AEGS_eQTL.tableOne, 
