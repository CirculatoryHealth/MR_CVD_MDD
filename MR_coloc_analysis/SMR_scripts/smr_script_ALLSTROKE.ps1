...
# - probe .besd file (here contain only one probe each)
...
$fileNumbers = 2..18  # Adjust this range as needed
...
foreach ($i in $fileNumbers) {
    ...
    ../smr-1.3.1-win --bfile ../../../ld-files/g1000_eur --gwas-summary ../../Data/ALLSTROKE/ALLSTROKE_COJO.txt --beqtl-summary `"$outputfile`" --peqtl-smr `"$pvalueThreshold`" --out `"$smrfile`"
